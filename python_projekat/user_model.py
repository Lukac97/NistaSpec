class User:
    ...
    def __str__(self):
        ...
        return f'User {self.str_username} is registered on locations ' \
               f'{locations}.'
    ...
    def __getitem__(self, str_location):
        if str_location in self.dct_passwords:
            ...
            print(f'User {self.str_username} is not registered on location '
                  f'{str_location}.')
            return None
    ...
    def __setitem__(self, str_location, password):
        if str_location not in self.dct_passwords:
            ...
            print(f'User {self.str_username} is already registered on '
                  f'location {str_location}.')

    def __delitem__(self, str_location):
        if str_location in self.dct_passwords:
            del self.dct_passwords[str_location]
        else:
            print(f'User {self.str_username} is not registered on '
                  f'location {str_location}.')
